import {
  OPFSAdapter,
  opfsAdapter,
} from "@/infrastructure/storage/opfs.adapter";
import {
  VideoStreamWriter,
  videoStreamWriter,
} from "@/infrastructure/storage/video-stream-writer";
...
export class WorkoutStorageService {
  ...
  async startVideoRecording(
    ...
    const fileWriter = await this.dependencies.opfsAdapter.getVideoFileWriter(
      workout.id
    );
  ...
  async stopRecording(workout: WorkoutEntity): Promise<void> {
    const { sizeInBytes } =
      await this.dependencies.videoStreamWriter.stopRecording();
  ...
  async getStoredWorkouts(): Promise<WorkoutSummary[]> {
    ...
    for (const workoutId of workoutIds) {
      const metadata = await this.dependencies.opfsAdapter.readMetadata(
        workoutId
      );
