...
describe("StopWorkoutUseCase", () => {
  ...
  it("throws if workout not active", () => {
    expect(() => useCase.execute(idleWorkout)).toThrow(
  ...
  it("stops workout and publishes event", () => {
    ...
    expect(event.data.stats.status).toBe("stopped");
  });
  ...
  it("sets workout end time", () => {
    ...
    expect(activeWorkout.endTime!.getTime()).toBeGreaterThanOrEqual(
      before.getTime()
    );
    expect(activeWorkout.endTime!.getTime()).toBeLessThanOrEqual(
      after.getTime()
    );
  });
